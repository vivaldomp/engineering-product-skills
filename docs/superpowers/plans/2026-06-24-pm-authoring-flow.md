...
**Goal:** Add a first-class import/bootstrap path (`pm-import`) plus a derive-then-confirm questioning mode so the suite can onboard existing docs without rewriting, and migrate pre-existing `.product/` docs to the Phase 3 front-matter convention.

**Architecture:** All deliverables are plugin markdown (skills, a command, a shared reference) plus one new convention-test file. No runtime scripts and no new dependencies. `pm-import` is a new skill that ingests/maps/reports only (never authors). Derive-then-confirm is a second mode in the shared `questioning-protocol.md` that the three builders branch into when source exists. The legacy migration is a confirmation-gated step in `pm-doc-sync`. Tests are markdown-structure assertions (regex over file contents), mirroring `tests/metadata-conventions.test.js` and `tests/traceability-conventions.test.js`.
...
- The new skill's front-matter `name` MUST equal its directory name (`pm-import`) — `validate-plugin.test.js` enforces this.
- `pm-import` is analysis only: it MUST NOT write `.product/prd/prd.md`, `.product/sdd/sdd.md`, or `.product/adr/*.md`. It writes only `.product/import-gap-report.md`.
...
### Task 1: `pm-import` skill + `/pm-import` command (B6)

**Files:**
- Create: `plugins/product-design-suite/skills/pm-import/SKILL.md`
- Create: `plugins/product-design-suite/commands/pm-import.md`
...
```markdown
---
description: Ingest existing PRD/SRS/ADR/SDD docs into the suite via pm-import
...
git commit -m "feat: pm-import skill + command for ingesting existing docs (B6)
...
- Modify: `plugins/product-design-suite/skills/pm-prd-builder/SKILL.md` (step 4)
- Modify: `plugins/product-design-suite/skills/pm-sdd-builder/SKILL.md` (step 3)
- Modify: `plugins/product-design-suite/skills/pm-adr-builder/SKILL.md` (step 3)
- Modify: `plugins/product-design-suite/skills/pm-product-workflow/SKILL.md` (step 2 detect-stage)
...
4. For any missing required section, ask questions following
   ...
   content from `pm-import`, or source supplied by the user — use **derive-then-
...
### Task 3: Legacy front-matter migration in `pm-doc-sync` (Phase 3 carryover)

**Files:**
- Modify: `plugins/product-design-suite/skills/pm-doc-sync/SKILL.md` (add a step)
- Test: `tests/import-conventions.test.js` (append)
...
git commit -m "feat: pm-doc-sync legacy front-matter migration (Phase 3 carryover)
